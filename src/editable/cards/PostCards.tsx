...
/** Bold text-forward feature card (no image — home cards stay image-free). */
  ...
  return (
    <Link href={href} data-reveal className="signal-on-dark group relative block overflow-hidden rounded-3xl bg-[var(--slot4-dark-bg)] p-8 text-white sm:p-10">
      ...
      <div className="relative">
        ...
        <h3 className="mt-6 max-w-2xl text-3xl font-extrabold leading-[1.08] tracking-[-0.03em] sm:text-4xl">{post.title}</h3>
...
/** Image-free release card used on the home rails. */
export function RailPostCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} data-reveal style={{ '--reveal-delay': `${(index % 4) * 80}ms` } as CSSProperties} className={`group ${dc.layout.minRailCard} block rounded-3xl border ${pal.border} ${pal.surfaceBg} p-6 ${pal.shadow} ${dc.motion.lift}`}>
      ...
      <h3 className="mt-5 line-clamp-3 text-xl font-extrabold leading-snug tracking-[-0.02em] transition group-hover:text-[var(--slot4-accent)]">{post.title}</h3>
...
/** List-style card used on the (image-supporting) article archive. */
export function ArticleListCard({ post, href, index }: { post: SitePost; href: string; index: number }) {
  return (
    <Link href={href} data-reveal className={`group grid min-w-0 gap-6 rounded-3xl border ${pal.border} ${pal.surfaceBg} p-5 ${pal.shadow} ${dc.motion.lift} sm:grid-cols-[260px_minmax(0,1fr)]`}>
      <div className="signal-zoom relative aspect-[16/10] overflow-hidden rounded-2xl bg-[var(--slot4-media-bg)]">
      ...
      <div className="min-w-0 self-center">
        ...
        <h2 className="mt-3 line-clamp-2 text-2xl font-extrabold leading-snug tracking-[-0.02em] transition group-hover:text-[var(--slot4-accent)]">{post.title}</h2>
